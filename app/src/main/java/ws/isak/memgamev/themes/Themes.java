...
public class Themes {
	...
	public static Theme createBirdsTheme() {
		...
		// 9 drawables (max needed for 18 tiles: 3 x 6 board) //TODO update to size max defined in BoardConfiguration
		for (int i = 1; i <= 9; i++) {	//TODO update this with more birds
			...
			Log.i (TAG, "method createBirdsTheme: adding URLS: curCard.setImageURI1: " + URI_DRAWABLE + String.format("bird_%d", i) + "a | curCard.setImageURI2: " + URI_DRAWABLE + String.format("bird_%d", i) + "b" + " | curCard.setAudioURI: " + URI_DRAWABLE + String.format("example%d", i) + " | curCard.getSampleDuration: " + curCard.getSampleDuration());
	...
	public static Theme createSpectrogramsTheme() {
		...
		// 9 drawables (max needed for 18 tiles: 3 x 6 board) //TODO update to size max
		for (int i = 1; i <= 9; i++) {
			CardData curCard = new CardData();
			curCard.setCardID(i);
            ...
            curCard.setSampleDuration (Music.getAudioDuration(Shared.context.getResources().getIdentifier(curCard.getAudioURI().substring(URI_AUDIO.length()), "raw", Shared.context.getPackageName())));
			Log.i (TAG, "method createSpectrogramsTheme: adding URLS: curCard.setImageURI1: " + URI_DRAWABLE + String.format("spectrogram_%d", i) + " | curCard.setAudioURI: " + URI_DRAWABLE + String.format("example%d", i) + " | curCard.getSampleDuration: " + curCard.getSampleDuration());
			theme.cardObjs.add(curCard);
		}
		return theme;
	}
